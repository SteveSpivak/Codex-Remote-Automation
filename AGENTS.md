...
- Use iPhone Shortcuts and private transport to return decisions to the broker.
...
- Use `cra-backend-developer` for broker, JSON-RPC, transcript, and replay work
- Use `cra-macos-engineer` for `.codex`, App Server lifecycle, launchd, and fallback tooling
- Use `cra-network-architect` for Shortcuts, Tailscale, SSH, and mobile transport
- Use `cra-security-specialist` for approval authenticity, transport hardening, and threat modeling
- Use `cra-test-engineer` for replay, KPI, resilience, and fallback reliability validation
...
- Do not describe `action_id`, `AXDescription`, AppleScript, or OCR as the primary CRA contract after the App Server pivot.
