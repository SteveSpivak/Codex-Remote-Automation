...
function createBridgeSecureTransport({ sessionId, relayUrl, deviceState, storePath }) {
  ...
  function handleClientHello(message, sendControlMessage) {
    ...
    if (handshakeMode === HANDSHAKE_MODE_QR_BOOTSTRAP && hasConflictingTrustedPhone(phoneDeviceId, phoneIdentityPublicKey)) {
      sendControlMessage(createSecureError({
        ...
        message: "This Mac is already paired with another iPhone. Reset pairing before pairing a new phone.",
      }));
      return;
    }
  ...
  function handleResumeState(message) {
    ...
    const missingEntries = outboundBuffer.filter((entry) => entry.bridgeOutboundSeq > lastAppliedBridgeOutboundSeq);
  ...
  function trimOutboundBuffer() {
    while (outboundBuffer.length > MAX_BRIDGE_OUTBOUND_MESSAGES || outboundBufferBytes > MAX_BRIDGE_OUTBOUND_BYTES) {
...
function encryptEnvelopePayload(payloadObject, key, sender, counter, sessionId, keyEpoch) {
  ...
  const ciphertext = Buffer.concat([cipher.update(Buffer.from(JSON.stringify(payloadObject), "utf8")), cipher.final()]);
